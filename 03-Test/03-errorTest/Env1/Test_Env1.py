...
while running:
    ...
    #update
    mC,regStatus=persons[0].goPos((lX,lY))       
    ...
    if chng==1:
        ...
        for j in range(0, personNum):
            ...
            predsErrRow.append(hipotenus(personStatus[j][1],(lacationINnv((xTahmin,yTahmin)))))
    ...
    if lX>1050:
        ...
        if lY>800:
            import numpy as np
            import pandas as pd            
            columns=["L3Err","L5Err","L9Err"]
            fmt=["%.1f","%.1f","%.1f"]            
            df = pd.DataFrame(predsErr, columns=columns)
            np.savetxt(current_directory+"Env1_Err.csv", df, fmt=fmt, delimiter=",", header=','.join(df.columns),comments='')
            running = False
            lY=50         
