...
with open('ac1.txt', 'r') as file:
    ac1 = [int(line.strip()) for line in file]

with open('ac2.txt', 'r') as file:
    ac2 = [int(line.strip()) for line in file]
    
    
with open('ac3.txt', 'r') as file:
    ac3 = [int(line.strip()) for line in file]
...
def valueCreate(SigVal,numL):
    ...
    if numL==5:
        normC=[[ -81.404, -50.365],[-81.404, -50.365],[-81.404, -50.365],[ -81.404, -50.365],[ -75.12899999999999, -48.183]]
    if numL==9:
        normC=[[-81.404, -50.365],[-79.988, -47.867],[ -81.404, -50.365],[-78.525, -49.067],[-81.404, -50.365],[ -79.988, -47.867],[-81.404, -50.365],[ -78.525, -48.486999999999995],[ -75.12899999999999, -48.183]]
...
personNum=3
beginPos=[(122, 98),(448, 202),(954, 498)]

persons=[]
for i in range(0, personNum):
    persons.append(Player(pos=beginPos[i]))
...
for j in range(0, personNum):
    ...
    locStatus.append(personLocStatus)
    rssiStatus.append(personRssiStatus)
...
while running:
    ...
    for i in range(0, personNum):
        if hareket>1000:
            ...
            personStatus.append(persons[i].update(int(actions[i][hareket])))
    ...
    for j in range(0, personNum):        
        for i in range(0, LRmodulNum): 
            locStatus[j][i][1]=hipotenus(LRmodul[i].rect.center,personStatus[j][1])
            if locStatus[j][i][1]!=locStatus[j][i][0]:
                chng=1 
    ...
    if chng==1:
        ...
        # print("hareket sayısı:",len(actions))
        for j in range(0, personNum):
            signal=[] 
            signal3=[] 
            signal5=[]
            signal9=[]            
            for i in range(0, LRmodulNum):
                locStatus[j][i][0]=locStatus[j][i][1]
                rssiStatus[j][i]=rssiCalc(locStatus[j][i][1],FREQ,GTX,GRX,rssiLoss(linerEqu(LRmodulPos[i][0],personStatus[j][1])))
                LRmodulData[i].update(locStatus[0][i][1],rssiStatus[0][i])
                signal9.append(rssiStatus[j][i])
                if(i==2 or i==4 or i==7 ):
                    signal3.append(rssiStatus[j][i])
                if(i==0 or i==2 or i==4 or i==6 or i==8 ):
                    signal5.append(rssiStatus[j][i])
    ...
    if len(personPATH[j])>1:
        for j in range(0, personNum):
            ...
            #     pygame.draw.line(screen, RED, list(personPATH[j])[i-1],list(personPATH[j])[i], 5)
            #     pygame.draw.line(screen, BLUE, list(personPATHPred9L[j])[i-1],list(personPATHPred9L[j])[i], 4)
                
            
            transparent_surface = pygame.Surface((100,100), pygame.SRCALPHA)
            pygame.draw.circle(transparent_surface, (0, 0, 255, 64), (50,50), 50)
            screen.blit(transparent_surface, (list(personPATHPred9L[j])[len(personPATHPred9L[j])-1][0]-50,list(personPATHPred9L[j])[len(personPATHPred9L[j])-1][1]-50))

    pygame.display.flip()
    
    if pygame.key.get_pressed()[pygame.K_s]:
        pygame.image.save(screen, 'ekran_goruntusu.png')
        rect = pygame.Rect(0, 0, 1100, 850) 
        subsurface = screen.subsurface(rect)
        pygame.image.save(subsurface, 'ekran_bolumu.png')
        
    # if pygame.key.get_pressed()[pygame.K_l]:
    #     with open('actions.txt', 'w') as file:
    #         for i in range(0, len(actions)):
    #             file.write(str(actions[i])+ '\n')
