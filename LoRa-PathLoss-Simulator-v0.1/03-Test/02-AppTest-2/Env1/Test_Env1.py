    ...
    if len(personPATH[j])>1:
        for j in range(0, personNum):
            for i in range(1, len(personPATH[j])):
                ...
                # pygame.draw.line(screen, BLACK, list(personPATHPred3L[j])[i-1],list(personPATHPred3L[j])[i], 4)
                # pygame.draw.line(screen, GREEN, list(personPATHPred5L[j])[i-1],list(personPATHPred5L[j])[i], 4)
